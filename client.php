...
$logStreamer  = new logStreamer($config, 'php://stdin', 'tcp://'.$config['target']);
			...
			if ($this->_config['compression'] === false) {
				...
				if ($this->_uncompressedBufferLen > $this->_config['readSize']) {
					...
					if ($this->_config['binary'] === true) {
						...
						$pos = @strpos($this->_uncompressedBuffer, "\n", $this->_config['readSize']);
					}
					if ($pos > 0) {
						...
						if ($compressData !== false) {
							...
							$this->_uncompressedBufferLen = substr($this->_uncompressedBufferLen, $pos);
